...
// Greedy word-wrap. Keeps the ORIGINAL font size intact and flows the text
// onto extra lines when a word/line is wider than its box — so an edited or
// converted run never overflows onto neighbouring text nor gets shrunk.
const wrapTextToWidth = (text, measure, size, maxWidth) => {
  const paragraphs = String(text == null ? '' : text).split('\n');
  if (!maxWidth || maxWidth <= 1) return paragraphs;
  const out = [];
...
export const applyPdfTextEdits = async (file, edits) => {
  ...
  for (const e of edits) {
    ...
    // cover the original glyphs across every wrapped line
    const left = e.xPt - 1;
    const right = Math.max(e.xPt + boxW, e.xPt + maxLineW) + 1;
    page.drawRectangle({
      x: left,
      y: e.yPt - size * 0.30 - (lines.length - 1) * lineHeight,
      width: right - left,
      height: size * 1.34 + (lines.length - 1) * lineHeight,
      color: hexToRgb(e.bg),
    });
    ...
    for (let li = 0; li < lines.length; li++) {
      ...
      try {
        page.drawText(str, { x: drawX, y, size, font, color: col });
      } catch (err) {
        ...
        try {
          ...
          page.drawText(str, { x: drawX, y, size, font: uni, color: col });
        } catch (err2) {
          page.drawText(str.replace(/[^\x20-\x7E]/g, '?'), { x: drawX, y, size, font, color: col });
        }
      }
      if (e.underline) {
        page.drawRectangle({ x: drawX, y: y - size * 0.12, width: w, height: Math.max(0.6, size * 0.06), color: col });
      }
    }
  }
  return docPdf.save();
